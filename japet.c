	...
	for (j = 0; j < scr[i].nbGroups; j++)
	{
	  ...
	  //Si on a cliqué a l'intérieur du dessin d'un groupe  
	  {
		...
		if((selectedGroup->sWindow[0] == NB_WINDOWS_MAX) | //ou si toutes les valeurs des neurones de ce groupe sont déjà affichées	     
		  ...
		  (selectedGroup->sWindow[3] == NB_WINDOWS_MAX && selectedGroup->function == "Picture")) //Regardé seulement si le groupe est une image		
...
void key_press_event (GtkWidget* pWidget, GdkEventKey *event)
{
  ...
  switch (event->keyval)
  {    
    case GDK_Page_Up: //Touche "Page up", on réduit le y du groupe sélectionné pour le faire monter      
    ...
    case GDK_Page_Down: //Touche "Page down", on augmente le y du groupe sélectionné pour le faire descendre
      if (selectedGroup != NULL)
      {
	do
	{
...
void askForScripts(GtkWidget* pWidget, gpointer pData)
{
  ...
  //-------------EXEMPLES DE SCRIPTS---------------------
    ...
    //Groupes du script 0
    newGroup(&scr[0].groups[0], &scr[0], "SCR0GR0", "Motor", 0.1, 16, 4, 4, 2, 0, 0);
    newGroup(&scr[0].groups[1], &scr[0], "SCR0GR1", "Motor", 0.1, 9, 3, 3, 2, 1, 0);
    scr[0].groups[1].previous[0] = &scr[0].groups[0];	
    newGroup(&scr[0].groups[2], &scr[0], "SCR0GR2", "Sensor", 0.1, 12, 4, 3, 2, 1, 0);
    scr[0].groups[2].previous[0] = &scr[0].groups[1]; 	
    newGroup(&scr[0].groups[3], &scr[0], "SCR0GR3", "Picture", 0.1, 20, 4, 5, 3, 2, 0);
    scr[0].groups[3].previous[0] = &scr[0].groups[1];
    scr[0].groups[3].previous[1] = &scr[1].groups[1]; 
    
    
    //Groupes du script 1
    newGroup(&scr[1].groups[0], &scr[1], "SCR1GR0", "Motor", 0.1, 16, 4, 4, 2, 0, 0);
    newGroup(&scr[1].groups[1], &scr[1], "SCR1GR1", "Motor", 0.1, 9, 3, 3, 2, 1, 0);
    scr[1].groups[1].previous[0] = &scr[1].groups[0];
    newGroup(&scr[1].groups[2], &scr[1], "SCR1GR2", "Sensor", 0.1, 12, 4, 3, 3, 1, 0);
    scr[1].groups[2].previous[0] = &scr[1].groups[1];      
    
    //Groupes du script 2
    newGroup(&scr[2].groups[0], &scr[2], "SCR2GR0", "Motor", 0.1, 16, 4, 4, 3, 1, 0);
...
printf("++ nbGroups de scr[0]: %d\n",scr[0].nbGroups);
  ...
  */
  ...
  for (i = 0; i < nbScripts; i++)   
     for (j = 0; j < scr[i].nbGroups; j++)
     {
       if (scr[i].groups[j].knownY == FALSE) findY(&scr[i].groups[j]);
     }
...
void newScript(script* s, gchar* name, gchar* machine, gint z, gint nbGroups)
{
    ...
    gint tailleName, tailleMachine;

    tailleName = strlen(name);    
    s->name = (gchar*)malloc((tailleName+1) * sizeof(gchar));            
    strncpy(s->name, name, tailleName);
    s->name[tailleName] = '\0';
...
void newGroup(group* g, script* myScript, gchar* name, gchar* function, gfloat learningSpeed, gint nbNeurons, gint rows, gint columns, gint y, gint nbLinksTo, gint firstNeuron)
{
  ...
  gint tailleName = strlen(name);  
  g->name = (gchar*)malloc((tailleName+1) * sizeof(gchar));          
...
void updateNeuron(neuron* n, gfloat s, gfloat s1, gfloat s2, gfloat pic)
{
  ...
  if (displayMode[1] == 'a') //Si on est en mode échantillonné ('a' est la deuxième lettre de "Sampled mode")
  {
   if (nbSnapshots == NB_BUFFERED_MAX) //Si le tableau "buffer" est plein, on oublie sa première ligne (la plus ancienne) et on recule chaque valeur d'une ligne
   {
     ...
     {
       n->buffer[0][i-1] = n->buffer[0][i];
       n->buffer[1][i-1] = n->buffer[1][i];
       n->buffer[2][i-1] = n->buffer[2][i];
       n->buffer[3][i-1] = n->buffer[3][i];
     }  
   } 
...
void dessinGroupe (cairo_t* cr, gint a, gint b, gint c, gint d, group* g, gint z, gint zMax)
{
  ...
  else cairo_set_source_rgb (cr, WHITE);
...
void findY(group* g)
{
  ...
  for (i = 0; i < nbScripts; i++)
  {  
      if (scr[i].z == g->myScript->z)
      {
	for (j = 0; j < scr[i].nbGroups; j++)
	{
	  if (scr[i].groups[j].knownY == TRUE && scr[i].groups[j].x == g->x)
	  {
	    freeY++;
	  }
	}  
